...
folder_path = 'dataset/preprocessed/mode0/'
...
print(f"95th percentile of execute_delay_ms: {upper_limit_delay} ms")
...
lower_limit_gain = filtered_df['gain_vs_threshold'].quantile(0.20)
upper_limit_gain = filtered_df['gain_vs_threshold'].quantile(0.80)
print(f"10th percentile of gain_vs_threshold: {lower_limit_gain}")
print(f"90th percentile of gain_vs_threshold: {upper_limit_gain}")
...
axes2[0].plot([y_test.min(), y_test.max()], [y_pred_rf.min(), y_pred_rf.max()], 'k--', lw=2)
...
axes2[1].plot([y_test.min(), y_test.max()], [y_pred_LR.min(), y_pred_LR.max()], 'k--', lw=2)
