...
for nstep in [10,15,20,25,30,35,40,45,50]:
    optionPrice,error=mcDKOCall(assetPrice,strike,upBarrier,lowBarrier,rebate,riskFree,sigma,expiration,nstep,nsample)
    x.append(log(fabs(expiration/nstep)))
    y.append(log(fabs(optionPrice-mbest)))
plt.plot(x,y,'ro')
plt.show()
slope,intercept=linearLeastSquaresFit.straightLineFit(x,y,len(x)-1)
print('loga =',intercept)
print('b =',slope)