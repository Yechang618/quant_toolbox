...
def compute_currency_ranks(interest_rate_df):
    ...
    for col in countries:
        df[col+'_rank'] = np.nan # Initialize rank columns
    
    # Compute monthly ranks (1 = lowest interest rate)
    # rank_df = df[['Date']].copy()
    rank_dict = {}
    rates_dict = {}
    # print(f"\nComputing monthly ranks for countries: {countries}")
    # print(f"rank_df: {rank_df.head()}")
    for i in range(len(df)):
        ...
        for col in countries:
            rate = row[col]
            rates.append((rate,col))
        rates.sort(key=lambda x: x[0])  # Sort by rate
        rank_dict[date] = {col: rank+1 for rank, (rate, col) in enumerate(rates)}
        for col in countries:
            df.loc[i, col+'_rank'] = rank_dict[date][col]
        rates_dict[date] = rates
        # print(f"\nProcessing date: {date.date()} with rates:\n{rates}") 

    rank_avg = []
    for col in countries:
        rank_avg.append((df[col+'_rank'].mean(), col))
    rank_avg.sort(key=lambda x: x[0])  # Sort by average rank
    # Compute average rank for each country
    # print("\n✓ Computed monthly ranks and average ranks for each country:")
    # print(df.head())
    # print(rank_avg)
    return countries, df, rank_avg


def answer_ranking_questions(df, ranks_avg):
    """
    ...
    funding_1 = ranks_avg[0][1]
    funding_1_rank = ranks_avg[0][0]
    ...
    funding_2 = ranks_avg[1][1]
    funding_2_rank = ranks_avg[1][0]
    ...
    invest_1 = ranks_avg[-1][1]
    invest_1_rank = ranks_avg[-1][0]
    ...
    invest_2 = ranks_avg[-2][1]
    invest_2_rank = ranks_avg[-2][0]
    ...
    print("-"*70)
    for rank, (avg_rank, country) in enumerate(ranks_avg, 1):
...
def create_carry_positions(countries, interest_rate_df, n_long_short=3):
    """
...
def answer_canada_position_question(positions_df):
    ...
    """
    avg_position_canada = positions_df['Canada_pos'].mean()
...
def compute_portfolio_returns(countries, df, exret_df):
    """
    ...
    merged = df.merge(exret_df, on='Date', suffixes=('', '_ret'))
    
    # Compute portfolio return for each month
    # Note: positions determined at beginning of month, returns realized during month
    portfolio_returns = []
    dates = []
    position_cols = [f'{c}_pos' for c in countries]
    for idx in range(1, len(merged)):
        # Positions from current row
        positions = merged.loc[idx - 1, position_cols].values.astype(float)
        # Returns from NEXT row (realized during the month)
        returns = merged.loc[idx, [f'{c}_ret' for c in countries]].values.astype(float)
        
        # Portfolio return = sum(w_i * r_i)
        port_ret = np.nansum(positions * returns)
        # portfolio_returns.append(port_ret)
        # dates.append(merged.loc[idx + 1, 'Date'])
        merged.loc[idx, 'Portfolio_Return'] = port_ret
        
    return merged


def compute_performance_metrics(merged, freq='monthly'):
    """
    Compute annualized performance metrics
    """
    returns = merged['Portfolio_Return'].dropna()
    ...
    print("="*70)
    print(f"\nSample period: {merged['Date'].min().date()} to {merged['Date'].max().date()}")
...
def main():
    ...
    df = interest_rate_df.copy()  # For debugging
    exret_df = data['ExRet']
    
    # Step 2: Currency ranking analysis
    print("\n[2/5] Computing currency rankings...")
    countries, df, ranks_avg = compute_currency_ranks(df)
    funding_1, funding_2, invest_1, invest_2 = answer_ranking_questions(df, ranks_avg)
    
    # Step 3: Plot US rank over time
    print("\n[3/5] Plotting US rank over time...")
    plot_us_rank_over_time(df)
    
    # # Step 4: Create carry trade positions
    print("\n[4/5] Creating carry trade positions (short 3 lowest, long 3 highest)...")
    df = create_carry_positions(countries, df, n_long_short=3)
    avg_canada_pos = answer_canada_position_question(df)
    
    # # Step 5: Compute portfolio returns and performance metrics
    print("\n[5/5] Computing portfolio returns and performance metrics...")
    portfolio_returns_df = compute_portfolio_returns(countries, df, exret_df)
